...
from src.core.exceptions import NotFoundError, DataValidationError
...
class CandidateService:
    ...
    async def create_candidate(self, candidate: CandidateCreate) -> Optional[Candidate]:
        """Create a new candidate."""
        candidate.email = candidate.email.strip()
        if await self.candidate_repository.get_by_email(candidate.email):
            raise DataValidationError("Candidate with this email already exists.")
    ...
    async def update_candidate(
        ...
        """Update a candidate."""
        statuses = ("applied", "screening", "interviewed", "offered", "hired", "rejected")
        if candidate.status and candidate.status not in statuses:
            raise DataValidationError(f"status should be one of them: {statuses}")
    ...
    async def generate_report(self, skip: int = 0, limit: int = 100) -> str:
        ...
        writer = csv.DictWriter(
            output,
            fieldnames=["id", "full_name", "email", "position", "experience",
                        "skills", "status", "created_at", "updated_at"]
        )
        writer.writeheader()

        cursor = self.candidate_repository.collection.find().skip(skip).limit(limit)
